...
data = read_data('G:\\GAT\\实验一FGT数据集节点分类\\data\\New Folder\\PTBR_xindeMHRWAE.csv')
data = pd.DataFrame(data)
data_zs = 1.0 * (data - data.mean()) / data.std()  # 数据标准化


model = KMeans(n_clusters=2)  # 分为K类, 并发4
...
d =  tsne[r[u'聚类类别'] == 1]
plt.plot(d[0], d[1], 'go', markersize=20)
# d =  tsne[r[u'聚类类别'] == 2]
# plt.plot(d[0], d[1], 'b*', markersize=20)
# d =  tsne[r[u'聚类类别'] == 3]
# plt.plot(d[0], d[1], 'k>', markersize=20)
plt.show()