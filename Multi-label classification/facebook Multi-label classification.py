...
def get_graph() -> nx.classes.graph.Graph:
    ...
    """
    data = pd.read_csv("data/facebook/edges.csv")
...
def get_target() -> np.array:
    ...
    """
    data = pd.read_csv("data/facebook/target.csv")
...
embedding_file = read_data('G:\\GAT\\实验一FGT数据集节点分类\\data\\facebook_xindeMHRWAE.csv')
...
print(f1_score(y_test,y_pred,labels=[0,1,2,3,],average='micro'))
